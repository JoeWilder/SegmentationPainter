from PyQt6.QtWidgets import QWidget, QGraphicsView, QGraphicsScene, QVBoxLayout, QGraphicsPolygonItem, QApplication  # fmt: skip
from PyQt6.QtGui import QPixmap, QImage, QColor, QPainter, QColor, QImageReader, QKeyEvent, QCursor  # fmt: skip
...
class ImageCanvas(QGraphicsView):
        ...
        if (
            ...
            if self.tool_mode == ToolMode.CREATE_MASK:
                ...
                if (
                    ...
                    if (
                        self.current_mask_manager is None
                        or self.current_mask_manager.hasNothingDisplayed()
                    ):
                        return
                ...
                mask_polygon = MaskItem(
                    self.mask_color, self.current_mask_manager, unique_point
                )
                ...
                editing_existing_polygon = (
                    self.current_mask_manager.getClickedPointsCount() > 1
                )

                if editing_existing_polygon:
                    mask_array = self.segment_agent.generateMaskFromPoints(
                        self.current_mask_manager.getClickedPoints()
                    )
                else:
                    mask_array = self.segment_agent.generateMaskFromPoint(
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.isPointInEditableArea(event.pos()):
            ...
            if self.middle_mouse_button_pressed:
                ...
                QApplication.setOverrideCursor(
                    QCursor(QPixmap("assets/icons/circle.png"))
                )
        else:
        ...
        self.display_bar.getCoordinateDisplayWindow().updateCoordinates(
            int(self.mouse_position_x), int(self.mouse_position_y)
        )
    ...
    def eventFilter(self, object: QObject, event: QEvent):
        if isinstance(event, QKeyEvent) and event.key() == Qt.Key.Key_Tab:
            ...
            if self.isPointInEditableArea(
                self.display_bar.getCoordinateDisplayWindow().getCoordinates()
            ):
                if self.tool_mode == ToolMode.CREATE_MASK:
                    QApplication.setOverrideCursor(Qt.CursorShape.CrossCursor)
                else:
